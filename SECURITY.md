...
- Without exact app trust, one MCP form approval enables desktop operations across apps for that MCP connection. Routine stop, backend idle cleanup, and errors preserve consent while invalidating app state and pending work. `computer_use_revoke` clears consent and stops work; disconnect/restart/reload clears it too. No approval is persisted to disk or shared with other connections. Decline, cancellation, missing UI support, and malformed native approval requests fail closed.
- Explicit session consent covers the observed native app-access form only during `get_app_state`: the configured `computer-use` server, exact `Allow ChatGPT to use [app]?` wording, empty form schema, and known metadata. Both the basic form and the observed high-risk-app warning variant are recognized; the initial connection-consent prompt includes the native warning about prompt injection/data theft/loss. Unknown risk levels or changed warning text are not silently accepted. The acceptance contains no persistence metadata. There is no stable upstream app-access kind field, so changed or unfamiliar shapes fall back to host approval. Exact-app launch trust alone does not grant this native consent. Other native prompts still require the host's response, and macOS permissions are never changed.
